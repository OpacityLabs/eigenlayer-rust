...
mod test {
    ...
    use crate::config::OpacityAVSConfigProperties;

    use super::{parse_config_file, Result};

    #[test]
    fn test_parse_config_file() {
        let location = "./config/opacity.config.yaml";
        let config: Result<OpacityAVSConfigProperties> = parse_config_file(location);
        assert!(
            config.is_ok(),
